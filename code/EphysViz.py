...
cfg_set('max_amplif_channels', 16) # RHD File hot fix for now
...
with dpg.window(
    ...
    with dpg.table(header_row=False, policy=dpg.mvTable_SizingFixedFit):
        ...
        with dpg.table_row():
            dpg.add_text('')
            dpg.add_text('', tag='spike_panels_subplot_label')
            dpg.add_text('')
        ...
        with dpg.subplots(
            ...
            for row in range(8):
                for col in range(4):
                    plot_size = 50
                    with dpg.plot(
                        label=f'Ch {row*4+col:02d} - {row+col} Spikes{" " * cfg_get("panel_label_spaces")}',
        ...
        with dpg.tab_bar(tag='tab_bar') as tb:
                    ...
                    with dpg.tab(label='Spikes', tag='spikes_tab'):
                        ...
                        with dpg.table(header_row=False, policy=dpg.mvTable_SizingFixedFit):
                            ...
                            dpg.add_table_column(width_stretch=True)
                            with dpg.table_row():
                                dpg.add_text('Exclude Period:')
                                dpg.add_checkbox(
                                    default_value=False,
                                    callback=excl_toggle_callback,
                                )
                        with dpg.group(tag='spike_panel_excl_range', show=False):
                            dpg.add_text("Period to Exclude:")
                            with dpg.plot(
                                height=75, width=-1, tag='spike_excl_range_plot', no_mouse_pos=True,
                                no_menus=True,

                            ):
                                dpg.add_plot_axis(
                                    dpg.mvXAxis,
                                    tag='xaxis_spk_excl_label_tag',
                                    no_gridlines=True,
                                    lock_max=True,
                                    lock_min=True,
                                )
                                ...
                                dpg.add_drag_line(
                                    ...
                                    default_value=100,
                                    tag='start_excl_drag',
                                    callback=start_excl_drag_callback
                                )
                                dpg.add_drag_line(
                                    label="End",
                                    color=[214, 39, 40, 255],
                                    ...
                                    default_value=300,
                                    tag='end_excl_drag',
                                    callback=end_excl_drag_callback
                                )
                        dpg.bind_item_font('spike_panel_excl_range', small_font)
...
dpg.bind_item_theme('spike_panel_excl_range', rounded_items_theme)
